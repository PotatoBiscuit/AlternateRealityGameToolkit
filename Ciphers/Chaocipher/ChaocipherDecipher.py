...
def permute_left(alphabet, index):
    """Permute the left (ciphertext) alphabet after decryption."""
...
def permute_right(alphabet, index):
    """Permute the right (plaintext) alphabet after decryption."""
...
def chaocipher_decrypt(ciphertext, left_alphabet, right_alphabet):
    """Decrypt ciphertext using the Chaocipher algorithm."""
    plaintext = ""
    ...
    for char in ciphertext:
        if char.upper().isalpha():
            ...
            # Find position of ciphertext character in left alphabet
            index = left_alphabet.index(char_upper)

            # Get plaintext character from right alphabet at same position
            plain_char = right_alphabet[index]
            ...
            # Permute both alphabets (identical to encryption)
...
decrypted_message = chaocipher_decrypt(encrypted_message, left_alphabet, right_alphabet)
